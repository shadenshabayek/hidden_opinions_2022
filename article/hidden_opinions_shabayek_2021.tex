...
%Law of motion
 ...
 \end{equation}
where 
%\begin{equation*} \Delta  \alpha_{i,{t-1}} =  \sum_{j \in N_i} \mathbbm{1}_{\{ j \text{ express} \}}\big( \mathbbm{1}_{ \{ |\Delta_{ij,t-1}| < \tau \} }  \Delta_{ji,t-1} + \mathbbm{1}_{ \{ |\Delta_{ij,t-1} | \geq \tau \} }  \Delta_{ij,t-1} \big) \text{ and } \alpha_{i,t} \in [-1,1] \end{equation*} 
...
%\begin{equation}  \alpha_{i,t} = \overline{\alpha}_{i,t-1} = \frac{1}{d_i} \big( g_{ii}  \alpha_{i,t-1} + \sum_{j \neq i \in N} g_{ji} {\alpha}_{j,t-1} \big) \label{average} \end{equation}
\begin{equation} 
\alpha_{i,t} = \overline{\alpha}_{i,t-1} = \frac{1}{d_i}  \sum_{j  \in N} g_{ji} {\alpha}_{j,t-1}. \label{average}
\end{equation}
 

%First, it allows for a pair of direct neighbors who express opinions which are close $enough$ (lower than a given threshold $\tau$) to update to an opinion which lies between both viewpoints. Second, a pair of direct neighbors who are ideologically-opposed (their opinion difference is higher than a given threshold $\tau$), update to opinions which drift farther and farther. 
